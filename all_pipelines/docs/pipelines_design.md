...
**Opset 18 — resolved.** Both `pipeline_ort_mobile.py` and `pipeline_tflite.py` now explicitly export at opset 18. The upgrade was effectively forced: the current `torch.onnx.export` (backed by `torch.export`) requires opset ≥ 18 and promotes the export automatically; the onnxscript version converter cannot downconvert this model back to opset 12. The code has been updated to match reality. There is no effect on benchmark results for MobileNet-V2's op set.